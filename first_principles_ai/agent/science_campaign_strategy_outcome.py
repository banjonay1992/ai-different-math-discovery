"""Science campaign strategy outcome assessor.

This layer checks whether a planned science campaign strategy actually changed
symbolic science/theory campaign state. It is a plain-data assessment layer
only: no empirical discovery claim, no sibling imports, and no checkpoint
ownership claim unless the local status capsule verifies one.
"""
...
SCIENCE_CAMPAIGN_STRATEGY_OUTCOME_LEDGER_KIND = 'ai_different.science_campaign_strategy_outcome_ledger'


def empty_science_campaign_strategy_outcome_ledger() -> dict[str, Any]:
    return {
        'schema_version': 1,
...
def load_science_campaign_strategy_outcome_ledger(path: str | Path | None) -> dict[str, Any]:
    if not path or not Path(path).exists():
        return empty_science_campaign_strategy_outcome_ledger()
    with Path(path).open('r', encoding='utf-8') as handle:
        ledger = json.load(handle)
    return validate_science_campaign_strategy_outcome_ledger(ledger)


def write_science_campaign_strategy_outcome_ledger(path: str | Path, ledger: dict[str, Any]) -> Path:
    validated = validate_science_campaign_strategy_outcome_ledger(ledger)
    output = Path(path)
...
def validate_science_campaign_strategy_outcome_ledger(ledger: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(ledger, dict):
        raise ValueError('science campaign strategy outcome ledger must be a JSON object')
    ...
    for field in ('outcome_records', 'strategy_rows'):
        values = ledger.get(field)
        if not isinstance(values, list) or not all(isinstance(item, dict) for item in values):
            raise ValueError(f'{field} must be objects')
    latest = ledger.get('latest', {})
    if not isinstance(latest, dict):
        raise ValueError('science campaign strategy outcome latest must be an object')
    validated = {
        ...
        'assessed_strategy_keys': _unique_strings(ledger['assessed_strategy_keys']),
...
def read_science_campaign_strategy_outcome_transcript(path: str | Path | None) -> dict[str, Any]:
    if not path:
...
def load_plain_json(path: str | Path | None) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError('plain science campaign strategy outcome input must be a JSON object')
    return value


def build_science_campaign_strategy_outcome_assessment(
    *,
    transcript_messages: list[dict[str, Any]],
    strategy_outcome_ledger: dict[str, Any],
    ...
    prior_strategy_outcome_ledger: dict[str, Any] | None = None,
    ...
    ledger = validate_science_campaign_strategy_outcome_ledger(strategy_outcome_ledger)
    ...
    prior_strategy_outcome = _valid_prior_strategy_outcome_or_empty(prior_strategy_outcome_ledger or {})
    ...
    rows = _extract_strategy_outcome_rows(
    ...
    if not new_messages and not source_is_new:
        selected = _noop_outcome('no new science campaign strategy outcome evidence or source ledger state')
    else:
        selected = _select_strategy_outcome(
            rows=rows,
            assessed_keys=ledger['assessed_strategy_keys'],
            project_owned_boundary=project_owned_boundary,
        )
    outcome_id = 'science_strategy_outcome_' + stable_digest({
    ...
    selected['campaign_strategy_outcome_id'] = outcome_id
    message = export_science_campaign_strategy_outcome_message(
        selected,
        rows=rows,
        runtime_memory_hash_state=runtime_memory_hash_state,
        project_owned_boundary=project_owned_boundary,
        source_hashes={
            'science_campaign_strategy_outcome_source_hash': source_hash,
            ...
            'prior_strategy_outcome_ledger_hash': prior_strategy_outcome.get('ledger_hash'),
        },
    )
    ...
    assessed_key = _strategy_outcome_key(selected)
    if selected['selected_outcome'] != 'summarize_noop':
        ledger['assessed_strategy_keys'] = _unique_strings(list(ledger['assessed_strategy_keys']) + [assessed_key])
    if new_messages:
...
def export_science_campaign_strategy_outcome_message(
    ...
    *,
    ...
    topic: str = 'ai_different.science_campaign_strategy_outcome',
) -> dict[str, Any] | None:
    if selected['selected_outcome'] == 'summarize_noop':
        return None
    recipient = validate_participant(selected.get('selected_recipient') or 'orchestrator')
    row = _find_row(rows, selected.get('strategy_id'), selected.get('scenario_id')) or {}
    ...
    body = {
        ...
        'campaign_id': selected.get('campaign_id'),
        'scenario_id': selected.get('scenario_id'),
        'planned_strategy': selected.get('planned_strategy'),
        'selected_outcome': selected['selected_outcome'],
        'selected_action': selected['selected_action'],
    ...
    return build_module_chat_message(
        ...
        tags=['ai_different', 'science_campaign_strategy_outcome', body['selected_outcome'], 'label_clean' if body['label_clean'] else 'label_review_needed'],
    )


def write_science_campaign_strategy_outcome_outbox_jsonl(path: str | Path, message: dict[str, Any] | None) -> Path:
    output = Path(path)
    output.parent.mkdir(parents=True, exist_ok=True)
...
def _valid_plain_or_empty(ledger):
    if not ledger:
        return {}
    if not isinstance(ledger, dict):
        raise ValueError('plain science campaign strategy outcome ledger must be a JSON object')
    return dict(ledger)


def _valid_prior_strategy_outcome_or_empty(ledger):
    if not ledger:
        return {}
    if ledger.get('ledger_kind') != SCIENCE_CAMPAIGN_STRATEGY_OUTCOME_LEDGER_KIND:
        raise ValueError('prior science campaign strategy outcome ledger has wrong ledger_kind')
    return validate_science_campaign_strategy_outcome_ledger(ledger)


def _extract_strategy_outcome_rows(existing, strategy_ledger, frontier_outcome, frontier, campaign, theory_memory, hypothesis, experiment, module_chat, messages):
    ...
    for row in list(strategy_ledger.get('strategy_rows') or []):
        _upsert_row(rows, _row_from_strategy_row(row))
...
def _row_from_strategy_record(record):
    return {
        ...
        'source_outcome_id': record.get('source_outcome_id'),
        'hypothesis_id': record.get('hypothesis_id'),
        'campaign_id': record.get('campaign_id'),
        'scenario_id': record.get('scenario_id') or record.get('campaign_id') or record.get('hypothesis_id'),
        'planned_strategy': record.get('selected_strategy'),
        'selected_outcome': None,
        'source_evidence_used': [],
        'observed_sibling_evidence': list(record.get('requested_sibling_evidence') or []),
        ...
        'before_theory_memory_state': record.get('before_theory_memory_state') or 'unknown',
        'after_theory_memory_state': record.get('after_theory_memory_state') or 'unknown',
...
def _row_from_strategy_row(row):
    return {
...
def _row_from_campaign(row):
    state = str(row.get('campaign_state') or row.get('status') or '')
    return {
...
def _row_from_theory_memory(row):
    ...
    return {
        ...
        'planned_strategy': row.get('selected_strategy') or 'promote_validated_hypothesis_to_theory_memory',
        'selected_outcome': 'validated_hypothesis_promoted_to_theory_memory',
        'source_evidence_used': [{'source': row.get('source') or 'theory_memory', 'status': status}],
        'observed_sibling_evidence': [],
        ...
        'after_theory_memory_state': row.get('after_theory_memory_state') or status,
        'checkpoint_boundary_state': row.get('checkpoint_boundary_state') or 'clean',
        'checkpoint_boundary_notes': list(row.get('checkpoint_boundary_notes') or []),
        'label_leaks': list(row.get('label_leaks') or []),
...
def _row_from_message(message):
    ...
    return {
        'strategy_id': body.get('strategy_id') or evidence.get('strategy_id'),
        ...
        'planned_strategy': selected_strategy,
        'selected_outcome': selected_outcome,
        'source_evidence_used': source,
        'observed_sibling_evidence': sibling,
        'promotion_state': body.get('promotion_state') or ('promoted' if selected_strategy == 'promote_validated_hypothesis_to_theory_memory' else 'none'),
        'retirement_state': body.get('retirement_state') or ('retired' if selected_strategy == 'retire_weak_hypothesis_route' else 'none'),
        'repair_state': body.get('repair_state') or ('open' if selected_strategy == 'reopen_theory_repair_cycle' else 'none'),
        'closure_state': body.get('closure_state') or ('closed' if selected_strategy == 'close_stable_science_campaign' else 'none'),
        'waiting_blocker_state': body.get('waiting_blocker_state') or ('waiting' if selected_strategy and selected_strategy.startswith('request_') and not sibling else 'resolved'),
        'before_theory_memory_state': body.get('before_theory_memory_state') or 'unknown',
        'after_theory_memory_state': body.get('after_theory_memory_state') or 'unknown',
...
def _upsert_row(rows, incoming):
    key = incoming.get('scenario_id') or incoming.get('strategy_id') or incoming.get('hypothesis_id') or incoming.get('campaign_id')
    if not key:
        return
    ...
    for field in ('strategy_id', 'frontier_outcome_id', 'frontier_id', 'source_outcome_id', 'hypothesis_id', 'campaign_id', 'scenario_id'):
...
def _finalize_row(row):
    ...
    strategy = row.get('planned_strategy')
...
def _select_strategy_outcome(*, rows, assessed_keys, project_owned_boundary):
    for row in rows:
    ...
    for row in rows:
        if row.get('planned_strategy') == 'promote_validated_hypothesis_to_theory_memory' or row.get('promotion_state') == 'promoted':
            return _outcome(row, 'validated_hypothesis_promoted_to_theory_memory', 'record_validated_hypothesis_promotion', 'broadcast')
    for row in rows:
        if row.get('planned_strategy') == 'retire_weak_hypothesis_route' or row.get('retirement_state') in {'retired', 'blocked'}:
            return _outcome(row, 'weak_hypothesis_route_retired', 'record_weak_route_retirement', 'broadcast')
    for row in rows:
        if row.get('planned_strategy') == 'request_funfun_formal_or_proof_clarification' and _has_sibling(row, 'funfun'):
            return _outcome(row, 'funfun_formal_or_proof_clarification_received', 'record_funfun_clarification_received', 'broadcast')
    for row in rows:
        if row.get('planned_strategy') == 'request_code_simulation_or_primitive_capability' and _has_sibling(row, 'code_module'):
            return _outcome(row, 'code_simulation_or_primitive_capability_received', 'record_code_capability_received', 'broadcast')
    for row in rows:
        if row.get('planned_strategy') == 'request_language_terminology_or_protocol_clarification' and _has_sibling(row, 'language_model_2'):
            return _outcome(row, 'language_terminology_or_protocol_clarification_received', 'record_language_clarification_received', 'broadcast')
    for row in rows:
        if row.get('planned_strategy') == 'reopen_theory_repair_cycle' or row.get('repair_state') == 'open':
            return _outcome(row, 'theory_repair_cycle_reopened', 'record_theory_repair_reopened', 'orchestrator')
    for row in rows:
        if row.get('planned_strategy') == 'close_stable_science_campaign' or row.get('closure_state') in {'closed', 'stable'}:
            return _outcome(row, 'stable_science_campaign_closed', 'record_stable_campaign_closed', 'orchestrator')
    for row in rows:
        if row.get('planned_strategy') == 'schedule_next_frontier_hypothesis':
            return _outcome(row, 'next_frontier_hypothesis_scheduled', 'record_next_frontier_hypothesis_scheduled', 'orchestrator')
    for row in rows:
        if _waiting_for_evidence(row) and _strategy_outcome_key_for(row, 'planned_science_campaign_strategy_waiting_for_evidence') not in set(assessed_keys):
            return _outcome(row, 'planned_science_campaign_strategy_waiting_for_evidence', 'wait_for_science_campaign_strategy_evidence', 'orchestrator')
    for row in rows:
        if row.get('planned_strategy') == 'record_no_measurable_science_gain' or row.get('selected_outcome') in {'record_no_measurable_science_gain', 'no_measurable_theory_frontier_gain'}:
            return _outcome(row, 'no_measurable_science_strategy_gain', 'record_no_measurable_science_strategy_gain', 'orchestrator')
    return _noop_outcome('no science campaign strategy outcome selected')
...
def _outcome(row, selected_outcome, action, recipient):
    return {
        ...
        'campaign_id': row.get('campaign_id'),
        'scenario_id': row.get('scenario_id'),
        'planned_strategy': row.get('planned_strategy'),
        'target_hypothesis': row.get('hypothesis_id'),
        'target_campaign': row.get('campaign_id') or row.get('scenario_id'),
        'required_followup_evidence': _required_evidence(selected_outcome),
        'source_evidence_used': list(row.get('source_evidence_used') or []),
...
def _required_evidence(outcome):
    mapping = {
        'planned_science_campaign_strategy_waiting_for_evidence': ['strategy_return_evidence'],
        'theory_repair_cycle_reopened': ['repair_outcome'],
    }
    return mapping.get(outcome, [])


def _theory_memory_delta(outcome, row):
    if outcome == 'validated_hypothesis_promoted_to_theory_memory':
        return 'validated hypothesis promotion observed in symbolic campaign state'
    if outcome == 'weak_hypothesis_route_retired':
        return 'weak/refuted hypothesis route retirement observed'
    if outcome == 'stable_science_campaign_closed':
        return 'stable science campaign closure observed'
    if outcome == 'no_measurable_science_strategy_gain':
        return 'record no safe science-strategy outcome gain'
    return row.get('planned_strategy') or outcome
...
def _state_counts(selected):
    ...
    mapping = {
        ...
        'validated_hypothesis_promoted_to_theory_memory': 'promote',
        'weak_hypothesis_route_retired': 'retire',
        'funfun_formal_or_proof_clarification_received': 'funfun',
        'code_simulation_or_primitive_capability_received': 'code',
        'language_terminology_or_protocol_clarification_received': 'language',
        'theory_repair_cycle_reopened': 'repair',
        'stable_science_campaign_closed': 'close',
        'next_frontier_hypothesis_scheduled': 'schedule',
        'planned_science_campaign_strategy_waiting_for_evidence': 'waiting',
        'no_measurable_science_strategy_gain': 'no_gain',
    }
...
def _source_hash(strategy_source, frontier_outcome, frontier, campaign, theory_memory, hypothesis, experiment, module_chat, prior_strategy_outcome, runtime_memory):
    return stable_digest({
        'strategy': strategy_source.get('ledger_hash'),
        ...
        'prior_strategy_outcome': prior_strategy_outcome.get('ledger_hash'),
...
def _find_row(rows, strategy_id, scenario_id):
    for row in rows:
...
def _strategy_outcome_key(selected):
    return _strategy_outcome_key_for(selected, selected.get('selected_outcome'))


def _strategy_outcome_key_for(row, outcome):
    return stable_digest({
        'strategy_id': row.get('strategy_id'),
        'scenario_id': row.get('scenario_id'),
        'hypothesis_id': row.get('hypothesis_id'),

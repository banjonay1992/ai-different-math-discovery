"""Science history-guided policy outcome assessor.

Assesses whether a history-guided science coordination policy produced useful
symbolic evidence after it was applied. This is candidate-not-causal
coordination accounting, not causal proof, not a science benchmark, and not a
...
SCIENCE_HISTORY_GUIDED_POLICY_OUTCOME_LEDGER_KIND = 'ai_different.science_history_guided_policy_outcome_ledger'


def empty_science_history_guided_policy_outcome_ledger() -> dict[str, Any]:
    return {
        'schema_version': 1,
...
def load_science_history_guided_policy_outcome_ledger(path: str | Path | None) -> dict[str, Any]:
    if not path or not Path(path).exists():
        return empty_science_history_guided_policy_outcome_ledger()
    with Path(path).open('r', encoding='utf-8') as handle:
        ledger = json.load(handle)
    return validate_science_history_guided_policy_outcome_ledger(ledger)


def write_science_history_guided_policy_outcome_ledger(path: str | Path, ledger: dict[str, Any]) -> Path:
    validated = validate_science_history_guided_policy_outcome_ledger(ledger)
    output = Path(path)
...
def validate_science_history_guided_policy_outcome_ledger(ledger: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(ledger, dict):
        raise ValueError('science history-guided policy outcome ledger must be a JSON object')
    if ledger.get('ledger_kind') != SCIENCE_HISTORY_GUIDED_POLICY_OUTCOME_LEDGER_KIND:
        raise ValueError('science history-guided policy outcome ledger has wrong ledger_kind')
    ...
    if not isinstance(latest, dict):
        raise ValueError('science history-guided policy outcome latest must be an object')
    validated = {
...
def read_science_history_guided_policy_outcome_transcript(path: str | Path | None) -> dict[str, Any]:
    if not path:
...
def load_plain_json(path: str | Path | None) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError('plain science history-guided policy outcome input must be a JSON object')
    return value


def build_science_history_guided_policy_outcome_assessment(
    *,
    ...
    ledger = validate_science_history_guided_policy_outcome_ledger(policy_outcome_ledger)
    ...
    source_hash = _source_hash(policy, interaction_history, ab_outcome, ab_probe, theory_memory, campaign, hypothesis, experiment, module_chat, prior_outcome, runtime_memory, hf_status)
    ...
    if not new_messages and not source_is_new:
        selected = _noop_outcome('no new science history-guided policy outcome evidence or source ledger state')
    else:
        selected = _select_outcome(rows=rows, project_owned_boundary=project_owned_boundary)
    outcome_id = 'science_history_guided_policy_outcome_' + stable_digest({
        'source_hash': source_hash,
        'new_message_ids': [module_chat_message_id(message) for message in new_messages],
        'outcome': selected['selected_outcome_class'],
        'policy_id': selected.get('science_history_guided_policy_id'),
    })[:16]
    selected['science_history_guided_policy_outcome_id'] = outcome_id
    message = export_science_history_guided_policy_outcome_message(
        selected,
        rows=rows,
        ...
        source_hashes={
            'science_history_guided_policy_outcome_source_hash': source_hash,
    ...
    if selected['selected_outcome_class'] != 'summarize_noop':
        ledger['assessed_policy_keys'] = _unique_strings(list(ledger['assessed_policy_keys']) + [_outcome_key(selected)])
    if new_messages:
    ...
    latest = {
        'science_history_guided_policy_outcome_id': outcome_id,
    ...
    record = {
        ...
        'observed_evidence': selected.get('observed_evidence'),
...
def export_science_history_guided_policy_outcome_message(
    ...
    *,
    ...
    topic: str = 'ai_different.science_history_guided_policy_outcome',
) -> dict[str, Any] | None:
    if selected['selected_outcome_class'] == 'summarize_noop':
        return None
    recipient = validate_participant(selected.get('selected_recipient') or 'orchestrator')
    row = _find_row(rows, selected.get('science_history_guided_policy_id'), selected.get('selected_ordering_key')) or {}
    leak_terms = label_leak_terms({'selected': selected, 'row': row})
    body = {
        'module': 'AI Different',
        'response_kind': 'science_history_guided_policy_outcome',
        'science_history_guided_policy_outcome_id': selected.get('science_history_guided_policy_outcome_id'),
        ...
        'observed_evidence': selected.get('observed_evidence') or [],
    ...
    return build_module_chat_message(
        ...
        tags=['ai_different', 'science_history_guided_policy_outcome', body['selected_outcome_class'], 'label_clean' if body['label_clean'] else 'label_review_needed'],
    )


def write_science_history_guided_policy_outcome_outbox_jsonl(path: str | Path, message: dict[str, Any] | None) -> Path:
    output = Path(path)
    output.parent.mkdir(parents=True, exist_ok=True)
...
def _valid_kind_or_empty(ledger, kind, record_field):
    ...
    if ledger.get('ledger_kind') != kind:
        raise ValueError(f'plain science history-guided policy outcome source has wrong ledger_kind: {ledger.get("ledger_kind")}')
    return dict(ledger)
...
def _valid_plain_or_empty(ledger):
    if not ledger:
        return {}
    if not isinstance(ledger, dict):
        raise ValueError('plain science history-guided policy outcome source must be a JSON object')
    return dict(ledger)
...
def _valid_prior_outcome_or_empty(ledger):
    if not ledger:
        return {}
    if ledger.get('ledger_kind') != SCIENCE_HISTORY_GUIDED_POLICY_OUTCOME_LEDGER_KIND:
        raise ValueError('prior science history-guided policy outcome ledger has wrong ledger_kind')
    return validate_science_history_guided_policy_outcome_ledger(ledger)


def _extract_policy_rows(existing, policy, interaction_history, ab_outcome, ab_probe, theory_memory, campaign, hypothesis, experiment, module_chat, messages):
    ...
    for record in list(policy.get('policy_rows') or []):
        _upsert_row(rows, _row_from_source(record, 'policy_row'))
...
def _row_from_source(source, lineage):
    ...
    policy_id = source.get('science_history_guided_policy_id') or source.get('source_policy_id')
    ordering = _ordering_key_from_source(source)
    selected = source.get('selected_outcome_class') or source.get('selected_outcome') or source.get('selected_action') or source.get('status') or source.get('evidence_gate')
    signal = _signal_from_value(selected, source)
    counts = {'candidate_win': 0, 'control_win': 0, 'waiting': 0, 'no_gain': 0, 'regression': 0, 'boundary': 0}
    if signal in counts:
        counts[signal] += 1
    ...
    return {
        ...
        'source_policy_ids': _unique_strings(list(source.get('source_policy_ids') or []) + ([policy_id] if policy_id else [])),
        ...
        'observed_evidence': _unique_strings(list(source.get('observed_evidence') or []) + ([str(selected)] if selected else [])),
...
def _upsert_row(rows, incoming):
    key = incoming.get('science_history_guided_policy_id') or incoming.get('selected_ordering_key') or stable_digest(incoming)[:12]
    current = rows.setdefault(str(key), {
        'science_history_guided_policy_id': incoming.get('science_history_guided_policy_id'),
    ...
    for field in ('science_history_guided_policy_id', 'selected_ordering_key'):
        current[field] = current.get(field) or incoming.get(field)
    for field in ('source_policy_ids', 'source_interaction_history_ids', 'target_campaign_ids', 'target_hypothesis_ids', 'target_simulation_ids', 'observed_evidence', 'tested_commits', 'tested_tests', 'checkpoint_boundary_notes', 'label_leaks', 'lineage'):
...
def _finalize_row(row):
    ...
    row['policy_outcome_row_hash'] = stable_digest({
        'policy': row.get('science_history_guided_policy_id'),
...
def _select_outcome(*, rows, project_owned_boundary):
    ...
    for row in rows:
        if row.get('selected_ordering_key') == 'code_simulation_before_science' and _count(row, 'candidate_win'):
            return _outcome(row, 'retained_code_simulation_ordering_improved_science_campaign', 'retained')
    for row in rows:
        if row.get('selected_ordering_key') == 'proof_before_hypothesis' and _count(row, 'candidate_win'):
            return _outcome(row, 'retained_proof_before_hypothesis_ordering_improved_campaign', 'retained')
    for row in rows:
        if row.get('selected_ordering_key') == 'language_summary_before_science' and _count(row, 'candidate_win'):
            return _outcome(row, 'retained_language_summary_ordering_improved_campaign', 'retained')
    for row in rows:
        if row.get('selected_ordering_key') == 'history_policy_memory_before_science' and _count(row, 'candidate_win'):
            return _outcome(row, 'retained_history_policy_memory_ordering_improved_coordination', 'retained')
    for row in rows:
        if _count(row, 'control_win') or _count(row, 'regression'):
            return _outcome(row, 'control_or_regression_outperformed_history_policy', 'weakened')
    for row in rows:
        if _count(row, 'waiting'):
            return _outcome(row, 'policy_underpowered_waiting_for_more_evidence', 'waiting')
    for row in rows:
        if _count(row, 'no_gain') >= 2:
            return _outcome(row, 'retire_or_weaken_history_policy_after_repeated_no_gain', 'retired')
    for row in rows:
        if _count(row, 'no_gain'):
            return _outcome(row, 'record_no_measurable_history_policy_gain', 'weakened')
    return _noop_outcome('no science history-guided policy outcome selected')
...
def _outcome(row, selected_class, retention):
    return {
        ...
        'science_history_guided_policy_id': row.get('science_history_guided_policy_id'),
        ...
        'selected_ordering_key': row.get('selected_ordering_key'),
        ...
        'observed_evidence': list(row.get('observed_evidence') or []),
        ...
        'candidate_not_causal_wording': 'Candidate-not-causal history-guided policy outcome: observed symbolic evidence can guide retention, weakening, or retirement, not prove causality.',
        'no_overclaiming_proof': 'candidate-not-causal science history-guided policy outcome only; no causal proof, no science benchmark claim, and no project-owned checkpoint claim without local verification',
...
def _noop_outcome(reason):
    return {
        ...
        'science_history_guided_policy_id': None,
        'source_policy_ids': [],
        'source_interaction_history_ids': [],
...
def _state_counts(selected):
    ...
    mapping = {
        ...
        'control_or_regression_outperformed_history_policy': 'weakened',
        'policy_underpowered_waiting_for_more_evidence': 'waiting',
        'retire_or_weaken_history_policy_after_repeated_no_gain': 'retired',
...
def _ordering_key_from_source(source):
    ...
    if explicit:
        return str(explicit)
    selected = source.get('selected_policy_decision') or source.get('selected_action') or source.get('selected_outcome_class') or ''
...
def _signal_from_value(value, source):
    mapping = {
        ...
        'applied_ordering_candidate_win': 'candidate_win',
        'control_win': 'control_win',
        'control_better': 'control_win',
        'control_or_regression_outperformed_history_policy': 'control_win',
...
def _find_row(rows, policy_id, ordering_key):
    for row in rows:
        if policy_id and row.get('science_history_guided_policy_id') == policy_id:
            return row
